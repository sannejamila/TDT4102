    ...
    : holderName{holderName}, insType{insType}, value{value}, id{id}, insuranceText{insText}
{
    if (insuranceText == "Text missing!")
    {
        ...
        while( k < 10){
            string letter = svadaGenerator(all_groups);
            streng += letter;
            k+= 1;
        }

        insuranceText = streng;
...
string insuranceTypeToString(InsuranceType t)
{
    ...
    map<InsuranceType, string> Insurance_type_to_string {
        {InsuranceType::Car, "Car"}, {InsuranceType::Contents, "Contents"},
        {InsuranceType::Travel, "Travel"}
        };
    
    string streng = Insurance_type_to_string.at(t);
